...
function CheapsharkID() {
  const [status, setStatus] = useState(null); // Estado para almacenar el resultado de la prueba

  useEffect(() => {
    const testAPI = async () => {
      try {
        const response = await fetch("http://127.0.0.1:5020/api/v1/cheapshark/gameid?title=doom");
        if (response.ok) {
